...
static void emit_footer(void) {
    menu_printf(ANSI_GOTO_FMT, render_footer_anchor_line());
...
static void cmd_reset(void) { g_cmd.active = false; g_cmd.len = 0; }
...
static void input_key(char k) {
    ...
    if (g_input.pos < sizeof g_input.buf - 1
        && input_acceptable(g_input.item->input_type, k)) {
...
static const char *reserved_role(char k) {
    switch (k) {
        case KEY_REFRESH: return "built-in refresh";
        case KEY_BACK:    return "built-in back";
        case KEY_PATH:    return "built-in path";
        case KEY_CMD:     return "built-in cmd";
        default:          return NULL;
    }
}
