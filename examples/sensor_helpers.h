...
#define READ_F(name, expr, fmt, status_expr)                            \
    static void rd_##name(char *b, size_t n, atc_menu_status_t *st) {   \
        snprintf(b, n, fmt, (double)(expr));                            \
        *st = (status_expr);                                            \
    }

/** OK if @p v is in [lo,hi], else WARN. */
static inline atc_menu_status_t st_range(float v, float lo, float hi) {
    return (v >= lo && v <= hi) ? ATC_MENU_ST_OK : ATC_MENU_ST_WARN;
}

/** OK below @p warn, WARN below @p err, ERR otherwise. */
static inline atc_menu_status_t st_max(float v, float warn, float err) {
    if (v >= err)  return ATC_MENU_ST_ERR;
    if (v >= warn) return ATC_MENU_ST_WARN;
    return ATC_MENU_ST_OK;
}

/** OK above @p warn, WARN above @p err, ERR otherwise. */
static inline atc_menu_status_t st_min(float v, float warn, float err) {
    if (v <= err)  return ATC_MENU_ST_ERR;
    if (v <= warn) return ATC_MENU_ST_WARN;
    return ATC_MENU_ST_OK;
}
